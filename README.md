...
- `设备查询` 会先按 O20 协议轮询左右手节点，识别到 O20 后记录该 DEV 对应的左右手节点；未识别为 O20 的设备再按 L30 新协议读取 DeviceInFo。
...
多实例运行时，用不同端口启动，例如 `--port 8098` 和 `--port 8099`。普通“连接所选”不会抢占其他进程已经打开的 CANFD；如果需要接管，前端提供“强制连接”，会显式尝试 `CAN_CloseDevice` 后重新打开。该行为是否能跨进程生效取决于厂商驱动，如果驱动拒绝释放，仍会返回失败。
